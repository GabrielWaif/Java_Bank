...
public class Client extends BankAccount {
  ...
  public void deposit(double value, String depositorName) {
    if (value <= 0) System.out.println(
      "Cannot deposit a value that is less or equal to 0"
    ); else {
      this.balance += value;
      BankStatement buffer = new BankStatement(
        1,
        depositorName,
        this.fullName,
        value
  ...
  public void transfer(double value, int reciverId) {
    if (value <= 0) System.out.println(
      "Cannot transfer a value that is less or equal to 0"
    ); else {
      Client reciver = bank.find(reciverId);
      this.balance -= value;
      System.out.println(
        "Value of " + value + " transfered to " + reciver.getFullName()
      );
      BankStatement buffer = new BankStatement(
        2,
        this.fullName,
